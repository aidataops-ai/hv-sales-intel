...
def test_state_collapses_github_state(
    ...
    assert resp.json() == {"state": "paused", "github_state": "disabled_manually"}
...
def test_stop_disables_workflow_and_cancels_runs(
    ...
    assert resp.json() == {"ok": True, "state": "paused", "cancelled_runs": 2}
...
def test_resume_enables_workflow(
    ...
    assert resp.status_code == 200
    assert resp.json() == {"ok": True, "state": "active"}
